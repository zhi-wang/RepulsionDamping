...
static void drive(char charCode, double darr[3])
{
   float sarr[3] = {darr[0], darr[1], darr[2]};
   printf("%c  (r,ai,aj)    %16.8lf%16.4lf%16.4lf\n", charCode, darr[0],
          darr[1], darr[2]);
   printf("%s\n", "SINGLE");
   runs(charCode, sarr);
   printf("%s\n", "DOUBLE");
...
#define DEFAULT_DATA                                                           \
   "R   4.2680   4.3189   8.5908271422   -0.5636998917   -0.8034106213   "     \
...
int main(int argc, char** argv)
{
   ...
   while (fgets(line, LINE_LEN, inputFile)) {
      ...
      int cnt;
      char c;
      cnt = sscanf(line, "%c%lf%lf%lf%lf%lf%lf%lf%lf", &c, p, p + 1, p + 2,
                   p + 3, p + 4, p + 5, p + 6, p + 7);
      if (cnt == 9 || cnt == 4) {
         if (cnt == 9) {
